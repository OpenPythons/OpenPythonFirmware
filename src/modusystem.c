...
STATIC mp_obj_t usystem_syscall(mp_obj_t obj) {
    ...
    void *result = (void *) __syscall2(SYS_INVOKE, (int) buf, (int) len);
...
STATIC mp_obj_t usystem_methods(mp_obj_t obj) {
    size_t len = 0;
    const char *buf = mp_obj_str_get_data(obj, &len);
    void *result = (void *) __syscall3(SYS_REQUEST, SYS_REQUEST_METHODS, (int) buf, (int) len);
    return parse_2(result);
}
...
STATIC mp_obj_t usystem_annotations(mp_obj_t obj) {
    size_t len = 0;
    const char *buf = mp_obj_str_get_data(obj, &len);
    void *result = (void *) __syscall3(SYS_REQUEST, SYS_REQUEST_ANNOTATIONS, (int) buf, (int) len);
    return parse_2(result);
}
